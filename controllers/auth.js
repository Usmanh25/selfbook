...
export const register = async (req, res) => {
  try {
    ...
    // ✅ populate friends before sending back
    const populatedUser = await savedUser.populate("friends", "-password");
...
export const login = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }).populate("friends", "-password");
    if (!user) return res.status(400).json({ msg: "User does not exist." });
...
export const loginGuest = async (req, res) => {
  try {
    ...
    // ✅ populate friends for guest as well
    const populatedGuest = await guestUser.populate("friends", "-password");

    console.log("[LOGIN] Returning guest user:", {
      _id: populatedGuest._id,
      email: populatedGuest.email,
      friends: populatedGuest.friends,
    });

    const token = jwt.sign({ id: guestUser._id }, JWT_SECRET, { expiresIn: "7d" });

    const userObj = populatedGuest.toObject();
    delete userObj.password;
